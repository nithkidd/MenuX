...
export default function PublicMenu() {
  ...
  const loadMenu = async () => {
    ...
    try {
      ...
      console.log("🔍 Business name_km:", menuData.business.name_km);
      console.log("🔍 Business name:", menuData.business.name);
      setData(menuData);
    } catch (err: any) {
      console.error("Failed to load menu:", err);
      if (err.response?.status === 404) {
        setError(
          "Menu not found. Please check the URL or ensure the business is published.",
        );
            ...
            <div className="py-2 overflow-hidden">
              <div className="flex gap-2">
                {[1, 2, 3, 4].map((i) => (
                  <div
                    key={i}
                    className="h-9 w-24 bg-gray-100 rounded-xl animate-pulse"
                  />
                ))}
              </div>
            </div>
            ...
            <div key={cat} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {[1, 2, 3, 4].map((item) => (
                  <div
                    key={item}
                    className="h-32 rounded-2xl border border-gray-100 bg-white p-4 flex gap-4"
                  >
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 flex-col p-4">
        ...
        <button
          ...
          className="mt-4 text-stone-500 underline text-sm"
        >
          Retry
        </button>
      </div>
  ...
  const displayName =
    currentLang === "km" && business.name_km ? business.name_km : business.name;
  console.log("🌐 Current Language:", currentLang);
  console.log("🌐 Display Name:", displayName);
  console.log("🌐 Has Khmer Name:", business.name_km);
  ...
  return (
    <div
      className={`light min-h-screen bg-white pb-20 text-stone-900 ${fontClass}`}
    >
            ...
            {isSearchOpen ? (
              ...
              <>
                <div className="flex items-center gap-3 overflow-hidden flex-1">
                  ...
                  <h1
                    className={`text-xl font-bold text-gray-900 truncate ${fontClass}`}
                  >
                    {displayName}
                  </h1>
                </div>
                ...
                {showLanguageToggle && (
                  ...
                  >
                    ...
                    <span
                      className={`text-sm font-bold ${currentLang === "km" ? "font-khmer" : "font-english"}`}
                    >
      ...
      <footer className="bg-white border-t border-gray-100 mt-12 py-12">
        <div className="max-w-7xl mx-auto px-4">
                  ...
                  <div>
                    ...
                    <div className="flex gap-4">
                      ...
                      {business.website_url && (
                        <a
                          href={ensureAbsoluteUrl(business.website_url)}
                          target="_blank"
                          rel="noopener noreferrer"
          ...
          <div className="border-t border-gray-100 pt-8 text-center text-gray-400 text-sm">
            <p>
              &copy; {new Date().getFullYear()} {business.name}. Powered by
              Menux.
            </p>
          </div>
        </div>
